...
export function getAllAutomations() {
  ...
  return automations.map(automation => ({
    ...automation,
    people: getPeopleByAutomationId(automation.id),
    environments: getEnvironmentsByAutomationId(automation.id),
    test_data: getTestDataByAutomationId(automation.id),
    metrics: getMetricsByAutomationId(automation.id),
    artifacts: getArtifactsByAutomationId(automation.id)
  }));
}

export function getAutomationById(id) {
  const database = getDatabase();
  const automation = database.prepare('SELECT * FROM automations WHERE id = ?').get(id);
  ...
  const transaction = database.transaction(() => {
    ...
    const result = database.prepare(`
      INSERT INTO automations (
        air_id, name, type, brief_description, coe_fed, complexity, tool, tool_version,
    ...
    `).run(
      automation.air_id, automation.name, automation.type, automation.brief_description,
      automation.coe_fed, automation.complexity, automation.tool, automation.tool_version,
      automation.process_details, automation.object_details, automation.queue,
      automation.shared_folders, automation.shared_mailboxes, automation.qa_handshake,
      automation.preprod_deploy_date, automation.prod_deploy_date, automation.warranty_end_date,
      automation.comments, automation.documentation, automation.modified, automation.modified_by,
      automation.path
    );

    const automationId = result.lastInsertRowid;

    // Insert related data
    insertPeople(automationId, people);
    insertEnvironments(automationId, environments);
    insertTestData(automationId, test_data);
    insertMetrics(automationId, metrics);
    insertArtifacts(automationId, artifacts);

    return automationId;
  });

  const automationId = transaction();
